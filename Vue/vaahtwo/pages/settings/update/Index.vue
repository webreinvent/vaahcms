    ...
    <Panel class="is-small">
        <template class="p-1" #header>
            <div class="flex flex-row">
                <div>
                    <b class="mr-1">{{root.assets.language_string.update_settings.heading}}</b>
                </div>
            </div>
        </template>
        ...
        <template #icons>
            <div class="buttons">
                <Button icon="pi pi-refresh"
                        :label="root.assets.language_string.update_settings.check_for_update_button"
        ...
        <Message
            ...
            <div class="text-center" v-if="root.assets
                ...
                <p>{{root.assets.language_string.update_settings.current_version_of_vaahcms_is}}</p>
                <Button :label="root.assets.vaahcms.version"
                        data-testid="setting-notification_add_sms"

                        class="w-auto my-2 p-button-sm"
                />
                <p v-if="store.is_up_to_data"><span class="subtitle">{{root.assets.language_string.update_settings.check_for_update_message}}</span></p>
            </div>
        </Message>
        ...
        <div v-if="store.backend_update">
            <div v-if="store.release" class="text-sm">
                <Message class="py-2" icon="pi-sync pi"
                    ...
                    <p>{{root.assets.language_string.update_settings.a_newer_version}} <b>{{ store.remote_version }}</b> {{root.assets.language_string.update_settings.of_vaahcms_is_available}}</p>
                </Message>
                ...
                <Panel class="is-small">
                    <template class="p-1" #header>
                        <div class="flex flex-row">
                            <div>
                                <b class="mr-1">{{root.assets.language_string.update_settings.new_updates}}:</b>
                            </div>
                        </div>
                    </template>
                    ...
                    <div class="field-checkbox mt-3">
                        <Checkbox inputId="binary"
                                  ...
                                  :binary="true"
                                  class="is-small"/>
                        <label for="binary">{{root.assets.language_string.update_settings.new_updates_message}}</label>
                    </div>
                    <Button :label="root.assets.language_string.update_settings.update_now_button"
                    ...
                    <div class="grid m-0" v-if="store.is_update_step_visible">
                        <div class="col-3">
                            <ol class="pl-3">
                                <li class="mb-2">
                                    {{root.assets.language_string.update_settings.downloading_latest_version}}
                            ...
                            <Button :label="root.assets.language_string.update_settings.reload_button"
                                    icon="pi pi-refresh"
        ...
        <div v-if="store.manual_update">
            <div v-if="store.release">
                {{root.assets.language_string.update_settings.a_newer_version}} <b>{{store.remote_version}}</b> {{root.assets.language_string.update_settings.of_vaahcms_is_available}}
                {{root.assets.language_string.update_settings.new_updates_message}}
                <hr/>
                <b>{{root.assets.language_string.update_settings.new_updates}}:</b>
                <div class="content">

                    {{store.release.body}}
                </div>

                <b>{{root.assets.language_string.update_settings.steps_of_manually_upgrade}}</b>
                <ol class="ml-4">
