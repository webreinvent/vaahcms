...
<template>
    <div class="sidebar"  v-if="root && root.assets && root.assets.extended_views && root.assets.extended_views.sidebar_menu">

        <PanelMenu :model="root.assets.extended_views.sidebar_menu.success.vaahcms">
            <template #item="{item}">
                    ...
                    <div id="pv_id_4_0_header" class="p-panelmenu-header" tabindex="0" role="button" aria-label="File" aria-controls="pv_id_4_0_content">
                        <div class="p-panelmenu-header-content">
                            <a class="p-panelmenu-header-action" tabindex="-1" @click="expandNode(item)">
                                <span class="p-submenu-icon pi pi pi-chevron-down" :id="'arrow-icon' + item.index" v-if="item.items"></span>
